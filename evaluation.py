...
class FeatureMaker:
    ...
    def __init__(self, metadata, label_column="Ir", label_type='int', sample=50000):
...
def _evaluate_multi_classification(train, test, fake, metadata, eval):
    ...
    le = LabelEncoder()
    le.fit(y_trains[0])
    y_trains[0] = le.transform(y_trains[0])
    le.fit(y_valid)
    ...
    if eval is None:
        for model_spec in classifiers:
            ...
            for param in tqdm(param_set):
                ...
                size = [a["size"] for a in metadata["columns"] if a["name"] == "Ir"][0]
        ...
        for model_spec in classifiers:
            ...
            def _calc(best_model, x_test, y_test):
                best_scores = []
                for x_train, y_train in zip(x_trains, y_trains):
                    ...
                    if len(unique_labels) != len(np.unique(y_train)):
            ...
            best_f1_scores.append(_df(_calc(model_class(**params['param'][i]), x_test, y_test)))
            i+=1
...
def _evaluate_regression(train, test, fake, metadata, eval):
    ...
    if eval is None:
        for model_spec in regressors:
            ...
            for param in tqdm(param_set):
                model = model_class(**param)
                print((x_trains[0] == 0).all(axis=0))
                X = x_trains[0].copy()
                y = y_trains[0].copy()
                X += np.random.normal(0, 1e-8, size=X.shape)
        ...
        for model_spec in regressors:
            ...
            def _calc(best_model):
                best_scores = []
                for x_train, y_train in zip(x_trains, y_trains):
                    x_train += np.random.normal(0, 1e-6, size=x_train.shape)
                    y_train += np.random.normal(0, 1e-6, size=y_train.shape)
                    try:
                        best_model.fit(x_train, y_train)
